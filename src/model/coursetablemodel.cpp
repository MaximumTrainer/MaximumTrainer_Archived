...
//    qDebug() << "courseNameIs:" << course.getName();
    if (role == Qt::BackgroundRole && account->hashCourseDone.contains(course.getName())) {
    ...
    if (role == Qt::ForegroundRole && index.column() == 0 && course.getCourseType() == Course::USER_MADE) {
