...
//
// This ANTMsg class is to decode and Encode ANT Messages
//
// DECODING
 ...
 * we are dealing with (4e is an ANT broadcast message). So in the ANTMsg constructor we also
...
const char *
ANTMsg::channelEventMessage(unsigned char c)
{
    switch (c) {
...
ANTMsg::ANTMsg(const unsigned char *message) {
...
ANTMsg::ANTMsg(const unsigned char len,
...
ANTMsg ANTMsg::resetSystem()
{
    return ANTMsg(1, ANT_SYSTEM_RESET);
}

ANTMsg ANTMsg::assignChannel(const unsigned char channel,
                             const unsigned char type,
                             const unsigned char network)
{
    return ANTMsg(3, ANT_ASSIGN_CHANNEL, channel, type, network);
}

ANTMsg ANTMsg::unassignChannel(const unsigned char channel)
{
    return ANTMsg(1, ANT_UNASSIGN_CHANNEL, channel);
}

ANTMsg ANTMsg::setSearchTimeout(const unsigned char channel,
                                const unsigned char timeout)
{
    return ANTMsg(2, ANT_SEARCH_TIMEOUT, channel, timeout);
}

ANTMsg ANTMsg::requestMessage(const unsigned char channel,
                              const unsigned char request)
{
    return ANTMsg(2, ANT_REQ_MESSAGE, channel, request);
}

ANTMsg ANTMsg::setChannelID(const unsigned char channel,
                            const unsigned short device,
                            const unsigned char devicetype,
                            const unsigned char txtype)
{
    return ANTMsg(5, ANT_CHANNEL_ID, channel, device&0xff, (device>>8)&0xff, devicetype, txtype);
}

ANTMsg ANTMsg::setChannelPeriod(const unsigned char channel,
                                const unsigned short period)
{
    return ANTMsg(3, ANT_CHANNEL_PERIOD, channel, period&0xff, (period>>8)&0xff);
}

ANTMsg ANTMsg::setChannelFreq(const unsigned char channel,
                              const unsigned char frequency)
{
    ...
    // ANT_SPORT_FREQ = 57
    // ANT_KICKR_FREQ = 52
    return ANTMsg(2, ANT_CHANNEL_FREQUENCY, channel, frequency);
}

ANTMsg ANTMsg::setNetworkKey(const unsigned char net,
                             const unsigned char *key)
{
    return ANTMsg(9, ANT_SET_NETWORK, net, key[0], key[1], key[2], key[3], key[4], key[5], key[6], key[7]);
}

ANTMsg ANTMsg::setAutoCalibrate(const unsigned char channel,
                                bool autozero)
{
    return ANTMsg(4, ANT_ACK_DATA, channel, ANT_SPORT_CALIBRATION_MESSAGE,
                  ANT_SPORT_CALIBRATION_REQUEST_AUTOZERO_CONFIG,
                  autozero ? ANT_SPORT_AUTOZERO_ON : ANT_SPORT_AUTOZERO_OFF);
}

ANTMsg ANTMsg::requestCalibrate(const unsigned char channel)
{
    return ANTMsg(4, ANT_ACK_DATA, channel, ANT_SPORT_CALIBRATION_MESSAGE,
...
ANTMsg ANTMsg::boostSignal(const unsigned char channel)
{
    // [A4][02][6A][XX][57][9B]
    return ANTMsg(2, 0x6A, channel, 0x57);
}

ANTMsg ANTMsg::open(const unsigned char channel)
{
    return ANTMsg(1, ANT_OPEN_CHANNEL, channel);
}

ANTMsg ANTMsg::close(const unsigned char channel)
{
    return ANTMsg(1, ANT_CLOSE_CHANNEL, channel);
}

ANTMsg ANTMsg::tacxVortexSetFCSerial(const uint8_t channel, const uint16_t setVortexId)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, 0x10, setVortexId >> 8, setVortexId & 0xFF,
                  0x55, // coupling request
                  0x7F, 0, 0, 0);
}

ANTMsg ANTMsg::tacxVortexStartCalibration(const uint8_t channel, const uint16_t vortexId)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, 0x10, vortexId >> 8, vortexId & 0xFF,
                  0, 0xFF /* signals calibration start */, 0, 0, 0);
}

ANTMsg ANTMsg::tacxVortexStopCalibration(const uint8_t channel, const uint16_t vortexId)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, 0x10, vortexId >> 8, vortexId & 0xFF,
                  0, 0x7F /* signals calibration stop */, 0, 0, 0);
}

ANTMsg ANTMsg::tacxVortexSetCalibrationValue(const uint8_t channel, const uint16_t vortexId, const uint8_t calibrationValue)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, 0x10, vortexId >> 8, vortexId & 0xFF,
                  0, 0x7F, calibrationValue, 0, 0);
}

ANTMsg ANTMsg::tacxVortexSetPower(const uint8_t channel, const uint16_t vortexId, const uint16_t power)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, 0x10, vortexId >> 8, vortexId & 0xFF,
                  0xAA, // power request
...
ANTMsg ANTMsg::kickrErgMode(const unsigned char channel, ushort usDeviceId, ushort usWatts, bool bSimSpeed)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, // broadcast
...
ANTMsg ANTMsg::kickrSlopeMode(const unsigned char channel, ushort usDeviceId, ushort scale)
{
    return ANTMsg(7, ANT_BROADCAST_DATA, channel, // broadcast
...
ANTMsg ANTMsg::kickrSimMode(const unsigned char channel, ushort usDeviceId, float fWeight)
{
    // weight encoding
...
ANTMsg ANTMsg::kickrStdMode(const unsigned char channel, ushort usDeviceId, ushort eLevel)
{
    return ANTMsg(7, ANT_BROADCAST_DATA, channel, // broadcast
...
ANTMsg ANTMsg::kickrFtpMode(const unsigned char channel, ushort usDeviceId, ushort usFtp, ushort usPercent)
{
    return ANTMsg(9, ANT_BROADCAST_DATA, channel, // broadcast
...
ANTMsg ANTMsg::kickrRollingResistance(const unsigned char channel, ushort usDeviceId, float fCrr)
{
    // crr encoding
...
ANTMsg ANTMsg::kickrWindResistance(const unsigned char channel, ushort usDeviceId, float fC)
{
...
ANTMsg ANTMsg::kickrGrade(const unsigned char channel, ushort usDeviceId, float fGrade)
{

    qDebug() << "ANTMsg::kickrGrade";
...
ANTMsg ANTMsg::kickrWindSpeed(const unsigned char channel, ushort usDeviceId, float mpsWindSpeed)
{
    // windspeed encoding
...
ANTMsg ANTMsg::kickrWheelCircumference(const unsigned char channel, ushort usDeviceId, float mmCircumference)
{
...
ANTMsg ANTMsg::kickrReadMode(const unsigned char channel, ushort usDeviceId)
{
    return ANTMsg(5, ANT_BROADCAST_DATA, channel, // broadcast
                  KICKR_INIT_SPINDOWN, (unsigned char)usDeviceId, (unsigned char)(usDeviceId>>8)); // preamble
}

ANTMsg ANTMsg::kickrInitSpindown(const unsigned char channel, ushort usDeviceId)
{
    return ANTMsg(5, ANT_BROADCAST_DATA, channel, // broadcast
